...
(defn extract-georeferences
  [geoserver-data-dir uri]
  (if-let [gdal-info (run-gdal-info geoserver-data-dir uri)]
    (let [cols-rows-regex    #"(?s)Size is (\d+), (\d+)"
    ...
    (throw (Exception. (str "run-gdal-info failed with geoserver-data-dir=" geoserver-data-dir " and uri=" uri)))))
